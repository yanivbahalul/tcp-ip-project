...
<p>
קובץ ה-CSV <code>group68_http_input.csv</code> נוצר כקובץ קלט המכיל 41 הודעות DHCP המדמות תעבורת רשת אמיתית. 
...
<table>
    ...
    <tr>
        <td>message</td>
        <td>תוכן ההודעה (DHCP message)</td>
        <td>DHCP Discover - Client searching for DHCP server</td>
    </tr>
    <tr>
        <td>timestamp</td>
        <td>חותמת זמן (שניות מהתחלה)</td>
        <td>0.01, 0.026, 0.042...</td>
    </tr>
</table>

<p>
הקובץ מכיל 41 הודעות DHCP המדמות תהליך הקצאת כתובות IP ללקוחות: <strong>הודעות Discover</strong> - הלקוח מחפש שרת DHCP (מפורט 68 לפורט 67), <strong>הודעות Offer</strong> - השרת מציע כתובת IP (מפורט 67 לפורט 68, כתובות 192.168.1.100-109), <strong>הודעות Request</strong> - הלקוח מבקש את הכתובת שהוצעה (מפורט 68 לפורט 67), <strong>הודעות ACK</strong> - השרת מאשר את הקצאת הכתובת (מפורט 67 לפורט 68), <strong>הודעת Release</strong> - הלקוח משחרר כתובת IP (בהודעה מס' 22, transaction 5), ו-<strong>הודעת NAK</strong> - השרת דוחה בקשת כתובת (בהודעה מס' 33, transaction 8). הקובץ כולל 10 תרחישי DHCP (msg_id 1-10), כאשר רוב התרחישים כוללים את כל שלבי התהליך: Discover → Offer → Request → ACK. תרחיש 5 כולל גם Release, תרחיש 6 מתחיל ישירות ב-Offer (ללא Discover), ותרחיש 8 מסתיים ב-NAK במקום ACK.
</p>
...
<p>
<strong>שכבת היישום (Application Layer):</strong> הפונקציה לוקחת את תוכן ההודעה מה-CSV (DHCP message) ויוצרת מבנה dictionary עם פרוטוקול DHCP ו-payload (תוכן ההודעה).
</p>

<p>
<strong>שכבת התעבורה (Transport Layer - TCP):</strong> הפונקציה מגדירה פרוטוקול TCP, משתמשת בפורטים מה-CSV (src_port: 68, dst_port: 67) - פורט 68 מייצג את הלקוח ופורט 67 מייצג את שרת ה-DHCP, יוצרת מספר רצף (seq_num) ומספר אישור (ack_num) אקראיים, ומגדירה דגלי TCP: PSH, ACK.
</p>
...
<pre>
{
    ...
    "application": {
        "protocol": "DHCP",
        "payload": "DHCP Discover - Client searching for DHCP server"
    },
    "transport": {
        "protocol": "TCP",
        "src_port": 68,
...
מה שמסביר את המחיר של הארגון ההיררכי ואת החשיבות של פרוטוקולים יעילים.
</p>
...
<p>
תהליך הלכידה מתחיל בהכנת סביבת העבודה: הרצת Jupyter Notebook עם הקוד לשליחת חבילות TCP/IP ויצירת חבילות DHCP.
</p>

<p>
בשלב הגדרת Wireshark, יש לפתוח את Wireshark ולבחור את ממשק ה-Loopback (Npcap Loopback Adapter ב-Windows או lo ב-Linux/macOS), להגדיר פילטר <code>tcp.port == 12345 && ip.addr == 127.0.0.1</code> כפי שנקבע במחברת Jupyter, ולהתחיל לכידה (Start Capture).
</p>

<p>
שליחת התעבורה מתבצעת על ידי הרצת הקוד מה-notebook. הקוד קורא את קובץ ה-CSV ויוצר חבילות TCP/IP עם תוכן הודעות ה-DHCP, כאשר כל חבילה מכילה הודעת DHCP אחת. החבילות נשלחות דרך ממשק ה-Loopback (127.0.0.1) לפורט 12345 כפי שנקבע במחברת Jupyter.

<strong>הסבר על בחירת הפורט 12345:</strong>
פורט 12345 נקבע כפורט יעד במחברת Jupyter לצורך יצירת החבילות TCP/IP. פורט זה נבחר לצורך ההדגמה וההרצה של הפרויקט, והוא אינו קשור לפורטים ה-DHCP הסטנדרטיים (67 ו-68) המצויים בקובץ ה-CSV. בעוד שקובץ ה-CSV מכיל את פורטי ה-DHCP המקוריים (67 לשרת ו-68 ללקוח), המחברת Jupyter משתמשת בפורט 12345 כפורט יעד אחיד ליצירת כל החבילות TCP/IP, ללא קשר לתוכן הודעות ה-DHCP שבהן.

<strong>הערה חשובה:</strong> חשוב לציין שזה ידוע שפורטי ה-DHCP הסטנדרטיים הם 67 (שרת DHCP) ו-68 (לקוח DHCP), והם משקפים את הפרוטוקול המקורי. במהלך הפרויקט פעלנו בהתאם להנחיות המחברת Jupyter, שהגדירה את פורט 12345 כפורט יעד, ולכן הצילומי מסך ב-Wireshark בוצעו בהתאם למחברת ומוצגים עם הפורט 12345. כמו כן, בוצעה בדיקה נוספת על הפורטים המקוריים (67 ו-68) בחיפוש ב-loopback באמצעות פילטרים מתאימים, והגענו לאותם ממצאים מבחינת מבנה החבילות, שכבות הפרוטוקול, דגלי TCP, ותוכן ה-payload. זה מוכיח שהאריזה והניתוח הם נכונים ועקביים, ללא תלות בבחירת הפורט הספציפי, שכן הפורט משמש רק לצורך זיהוי החבילות ולא משפיע על מבנה השכבות והתוכן של הודעות ה-DHCP.
</p>
...
<p>
הקוד מיישם את השליחה באופן הבא: הוא קורא את קובץ ה-CSV עם pandas, עובר על כל שורה ויוצר חבילת TCP/IP עם תוכן הודעת ה-DHCP. עבור כל הודעה, הקוד יוצר חבילת TCP/IP מלאה (IPv4 + TCP headers) עם תוכן ההודעה מה-CSV, ושולח אותה דרך ממשק ה-Loopback (127.0.0.1) לפורט היעד 12345 כפי שהוגדר במחברת Jupyter. הפורט המקור נקבע אקראית בטווח 1024-65535. החבילות נשלחות באמצעות ספריית Scapy או raw sockets, תלוי במערכת ההפעלה. תוכן הודעות ה-DHCP מקובץ ה-CSV משמש כ-payload של החבילות, אך הפורט היעד הוא 12345 כפי שהוגדר במחברת, ולא פורטי ה-DHCP המקוריים (67/68).
</p>

<p>
הגישה של יצירת חבילות TCP/IP עם תוכן DHCP מדמה תעבורת DHCP אמיתית, 
שבה כל הודעה נשלחת כחבילה עצמאית.
</p>
...
<ol>
    ...
    <li><strong>העברת נתונים (DHCP):</strong>
        <p>
        נשלחות הודעות DHCP שונות:
        </p>
        <ul>
            <li>הודעות Discover: DHCP Discover - Client searching for DHCP server</li>
            <li>הודעות Offer: DHCP Offer - Server offering IP 192.168.1.100-109</li>
            <li>הודעות Request: DHCP Request - Client requesting IP</li>
            <li>הודעות ACK: DHCP ACK - Server acknowledging IP assignment</li>
            <li>הודעות Release: DHCP Release - Client releasing IP</li>
            <li>הודעות NAK: DHCP NAK - Server denying IP request</li>
        </ul>
        <p>
...
<pre>
דוגמה לתעבורה שנלכדה - חבילות TCP/IP עם תוכן DHCP שנלכדו דרך ממשק ה-Loopback. החבילות מכילות הודעות DHCP שונות (Discover, Offer, Request, ACK, Release, NAK) הנשלחות בין כתובת 127.0.0.1 לכתובת 127.0.0.1, כאשר פורט המקור הוא 68 (לקוח DHCP) ופורט היעד הוא 67 (שרת DHCP).
</pre>
</div>
...
<p>
<strong>Transmission Control Protocol (שכבת תעבורה)</strong> מכיל פורטים (src_port: 68, dst_port: 67), מספרי רצף ואישור, דגלים (SYN, ACK, FIN, RST, PSH), וגודל חלון (Window Size). <strong>Dynamic Host Configuration Protocol (שכבת יישום)</strong> מכיל הודעות DHCP (Discover, Offer, Request, ACK, Release, NAK) המדמות תהליך הקצאת כתובות IP ללקוחות.
</p>

<h4>תובנות מהניתוח:</h4>

<p>
התעבורה מציגה מספר תובנות: כל הודעה נשלחת כחבילת TCP/IP עצמאית עם תוכן DHCP. החבילות נשלחות דרך ממשק ה-Loopback (127.0.0.1) ומייצגות תעבורת DHCP המדמה תהליך הקצאת כתובות IP. חלק מהחבילות עשויות להכיל דגלי RST, המעידים על כך שאין שרת DHCP פעיל המאזין על הפורט, מה שמעיד על ההתנהגות התקינה של פרוטוקול TCP. חבילות DHCP קטנות יחסית, מה שמתאים להודעות בקרה קצרות.
</p>
...
<p>
<img src="img/wireshark_tcp_stream.png" alt="Follow TCP Stream - תוכן ההודעה DHCP">
</p>

<p>
הצילום הראשון מציג את התוצאה של "Follow TCP Stream" עבור stream מסוים. 
זה מציג את תוכן ההודעות המלא:
</p>

<p>
<strong>הודעות DHCP מהלקוח:</strong> הודעות DHCP Discover ו-Request הנשלחות מהלקוח (פורט 68) לשרת (פורט 67).
</p>

<p>
<strong>הודעות DHCP מהשרת:</strong> הודעות DHCP Offer ו-ACK הנשלחות מהשרת (פורט 67) ללקוח (פורט 68), כולל הצעות כתובות IP ואישור הקצאה.
</p>
...
<p>
צילום זה מדגים את תוכן ההודעות בפועל - הודעות DHCP המדמות תהליך הקצאת כתובות IP, 
    ...
    <li><strong>חלק אמצעי (פירוט החבילה):</strong> מציג את כל שכבות המודל TCP/IP:
        <ul>
            ...
            <li><strong>Internet Protocol Version 4 (שכבת רשת):</strong>
                <ul>
                    <li>Source IP: 127.0.0.1 (Loopback - מקומי)</li>
                    <li>Destination IP: 127.0.0.1 (Loopback - מקומי)</li>
                </ul>
            </li>
            <li><strong>Transmission Control Protocol (שכבת תעבורה):</strong>
                <ul>
                    <li>Source Port: 67 או 68 (פורט DHCP)</li>
                    <li>Destination Port: 67 (פורט שרת DHCP)</li>
...
משכבת היישום (DHCP) ועד שכבת הקו (Ethernet). ניתן לראות את כל ה-headers של כל שכבה, 
...
<p>
בחלק 1, הקוד קורא קובץ CSV עם הודעות DHCP ויוצר חבילות TCP/IP המכילות את תוכן ההודעות. החבילות נשלחות דרך ממשק ה-Loopback (127.0.0.1) לפורט 67 (שרת DHCP), מדמות תעבורת DHCP אמיתית.
</p>
...
[
  {
    ...
    "direction": "received",
    "message": "DHCP Discover - Client searching for DHCP server"
  },
  {
    "timestamp": "2026-01-02T12:34:56.234567",
    "client_id": "127.0.0.1:68",
    "direction": "sent",
    "message": "DHCP Offer - Server offering IP 192.168.1.100"
  }
]
</pre>
...
<ul>
    <li>החבילות מכילות הודעות DHCP עם PSH+ACK</li>
    <li>חבילות ACK מאשרות קבלת הודעות</li>
    <li>חלק מהחבילות מכילות דגלי RST, המעידים על היעדר שרת DHCP פעיל</li>
</ol>

<h4>שכבת היישום (Application Layer - DHCP):</h4>

<ul>
    <li><strong>הודעות DHCP מהלקוח:</strong>
        <ul>
            <li>DHCP Discover - Client searching for DHCP server</li>
            <li>DHCP Request - Client requesting IP address</li>
            <li>DHCP Release - Client releasing IP address</li>
        </ul>
    </li>
    <li><strong>הודעות DHCP מהשרת:</strong>
        <ul>
            <li>DHCP Offer - Server offering IP (192.168.1.100-109)</li>
            <li>DHCP ACK - Server acknowledging IP assignment</li>
            <li>DHCP NAK - Server denying IP request</li>
        </ul>
    </li>
</ul>
...
<table>
    ...
    <tr>
        ...
        <td>DHCP</td>
        <td>משתנה</td>
        <td>הודעות DHCP (Discover, Offer, Request, ACK, Release, NAK)</td>
    </tr>
    <tr>
        <td>תעבורה</td>
        <td>TCP</td>
...
שמבצעת אריזה של הודעת DHCP בשכבות מודל TCP/IP.
...
- ליצור dictionary היררכי עם שכבות:
  * Application: protocol="DHCP", payload=message
  * Transport: protocol="TCP", src_port=68 (מהקובץ), dst_port=67 (מהקובץ), 
...
כולל הסבר על כל שכבה (Ethernet, IP, TCP, DHCP) 
...
<ul>
    ...
    יצירת קובץ CSV עם הודעות DHCP, יצירת חבילות TCP/IP, ולכידת התעבורה ב-Wireshark לניתוח מעמיק</li>
